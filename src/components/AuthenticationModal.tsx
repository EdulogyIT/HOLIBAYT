    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md p-0 [&>button]:hidden">
        <Button
          variant="ghost"
          size="icon"
          className="absolute right-4 top-4 rounded-full h-8 w-8 z-50"
          onClick={onClose}
        >
          <X className="h-4 w-4" />
        </Button>
        ...
        <div className="p-6">
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="country">{t('country') || 'Country/Region'}</Label>
              <select
                id="country"
                value={countryCode}
              ...
              >
                <option value="+213">{t('algeria') || 'Algeria'} (+213)</option>
                <option value="+33">{t('france') || 'France'} (+33)</option>
                <option value="+1">{t('usa') || 'United States'} (+1)</option>
                <option value="+44">{t('uk') || 'United Kingdom'} (+44)</option>
              </select>
            </div>
            ...
            <p className="text-xs text-muted-foreground">
              {t('phoneVerificationNotice') || "We'll call or text you to confirm your number. Standard message and data rates apply."}
            </p>
            ...
            <div className="relative my-6">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-4 bg-background text-muted-foreground">{t('or') || 'or'}</span>
              </div>
            </div>
