...
import { Building2, Eye, Edit, Trash2, Plus, MapPin, Star, Calendar, MessageSquare, FileText } from 'lucide-react'; 
                ...
                <div className="space-y-3"> 
                  ...
                  <div className="text-sm text-muted-foreground"> 
                    <Badge variant="outline"> 
                      {property.category === 'sale' ? t('buy') : property.category === 'rent' ? t('rent') : t('shortStay')} 
                    </Badge> 
                  </div> 
                  ...
                  <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2 pt-2"> 
                    <Button size="sm" variant="outline" className="w-full sm:flex-1" onClick={() => navigate(/property/${property.id})}> 
                      <Eye className="h-4 w-4 mr-1" /> 
                      <span className="text-xs sm:text-sm">{t('host.view')}</span> 
                    </Button> 
                    <Button size="sm" variant="outline" className="w-full sm:flex-1" onClick={() => navigate(/edit-property/${property.id})}> 
                      <Edit className="h-4 w-4 mr-1" /> 
                      <span className="text-xs sm:text-sm">{t('host.edit')}</span> 
                    </Button> 
                    {property.category === 'rent' && ( 
                      <Button size="sm" variant="secondary" className="flex-1" onClick={() => navigate(/host/create-agreement?propertyId=${property.id})}> 
                        <FileText className="h-4 w-4 mr-1" /> Agreement 
                      </Button> 
                    )} 
                    <Button size="sm" variant="outline" className="text-red-600 hover:text-red-700" onClick={async () => { if (window.confirm(t('host.confirmDelete') || 'Are you sure you want to delete this property?')) { try { const { error } = await supabase .from('properties') .delete() .eq('id', property.id); if (error) { console.error('Error deleting property:', error); } else { fetchHostProperties(); // Refresh the list } } catch (error) { console.error('Error deleting property:', error); } } }}> 
